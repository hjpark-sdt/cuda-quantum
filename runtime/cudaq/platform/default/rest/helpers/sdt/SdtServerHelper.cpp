...
protected:
  /// @brief The base URL
  std::string baseUrl = "https://api.sdt.cloud:5000/";  // hjpark: 수정 필요
...
SdtServerHelper::createJob(std::vector<KernelExecution> &circuitCodes) {
  ...
  credentialsPath = searchAPIKey1(apiKey, refreshKey, credentials, timeStr,
  ...
  cudaq::info(
      "Created job payload for sdt, language is QIR 1.0, targeting {}",
      machine);
